...
export class ManageProductsComponent implements OnInit {
  ...
  variatIndex: any;
  subIndex: any;
  sameparent:boolean = false;

  sub: boolean;
  SelectedVariationID :any[] = [];
  variationSecondList :any[] = [];
  secondVariationsSize = '0'
  sizeSecondary = '0'
  second_variation:any[] = [];
  SelectedSecondaryVariationID:any[] = [];
  constructor(
  ...
  getVariation() {
    ...
    this.api.post('variation/getVariantByStores', param).then((data: any) => {
      if (data && data.status === 200) {
        this.variationList = data.data;
        console.log('this.variationList====>',this.variationList);
      }
    }, error => {
      console.log(error);
  ...
  getSecondVariation() {
    ...
    this.api.post('SecondVariation/getByUid', param).then((data: any) => {
      if (data && data.status === 200) {
        this.variationSecondList = data.data;
        console.log('this.variationSecondList',this.variationSecondList)
      }
    }, error => {
      console.log(error);
      this.error(this.util.getString('Something went wrong'));
  ...
  getProduct() {
    ...
    this.api.post('products/getById', param).then((data: any) => {
      ...
      if (data && data.status === 200 && data.data.length) {
        ...
        if (info && info.second_variation && info.second_variation !== '') {
          if (((x) => { try { JSON.parse(x); return true; } catch (e) { return false } })(info.second_variation)) {
            ...
            if(this.second_variation.length > 0){
              this.sizeSecondary = '1'
            }
          }
        }
        for(let i=0 ; i < this.variations[0].items.length ;i++){
            this.SelectedVariationID.push( this.variations[0].items[i].vid)
        }
        for(let i=0 ; i < this.second_variation.length ;i++){
          this.SelectedSecondaryVariationID.push( this.second_variation[i].id);
          console.log('this.SelectedSecondaryVariationID',this.SelectedSecondaryVariationID)
      }
        if (info.images) {
  ...
  addVariation(event,items,index){
    let obj = {
      vid:items.id,
      title:items.items_title,
      price:items.price,
      image:items.image,
      hb_title:items.hb_title,
    }
     if(event.target.checked){
      this.variations[0].items.push(obj)
     }else{
      const index = this.variations[0].items.findIndex((element, index) => {
        if (element.title === obj.title && element.price === obj.price) {
          return true
        }
      })
      if (index > -1) {
         this.variations[0].items.splice(index, 1);
      }
     }
  }
  addSecondaryVariation(event,items,index){

     if(event.target.checked){
      this.second_variation.push(items)
     }else{
      const index = this.second_variation.findIndex((element, index) => {
        if (element.en_cat_title === items.en_cat_title && element.hb_cat_title === items.hb_cat_title) {
          return true
        }
      })
      if (index > -1) {
         this.second_variation.splice(index, 1);
      }
     }
     console.log('this.second_variation',this.second_variation)
  }
  delete(item) {
    console.log(item);
    if (item.title === 'size') {
  ...
  create() {
    const image = [
      ...
      this.image3 ? this.image3 : 'NULL',
      this.image4 ? this.image4 : 'NULL',
      this.image5 ? this.image5 : 'NULL',
      this.image6 ? this.image6 : 'NULL'
    ];
    ...
    const param = {
      ...
      second_variation: JSON.stringify(this.second_variation)
    };
  ...
  update() {
    ...
    this.api.post('products/editList', param).then((data: any) => {
      ...
      if (data && data.status === 200) {
        ...
        this.error(this.util.getString(data.data.error));
      }else{
        this.error(this.util.getString('Something went wrong'));
      }
    }, error => {
  ...
  changeSecondarySize(event){

     this.sizeSecondary = event;
     if (this.sizeSecondary === '0') {
