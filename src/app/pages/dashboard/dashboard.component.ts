...
export class DashboardComponent implements OnInit {
    ...
    if (uid && uid !== null && uid !== 'null') {
      ...
      this.api.post('stores/getByUid', param).then((data: any) => {
        ...
        if (data && data.status === 200 && data.data && data.data.length) {
          ...
          console.log('getByUid',data)
          if(!data.data[0].delivery_range){
            this.delivery_range = 10
          }else{
            this.delivery_range = data.data[0].delivery_range;
          }
  ...
  close() {
    ...
    this.dummyDrivers.forEach( element => {
...
/**  Delivery Range add here in store table  */

setDeliveryRange(){

  if(!this.delivery_range || this.delivery_range == '' || this.delivery_range == null){
    this.error(this.util.getString('Delivery range is required!'));
    return;
  }
  const param = {
    id: this.util.storeInfo.id,
    delivery_range:this.delivery_range
    
  }
  this.spinner.show();
  this.api.post('stores/editList', param).then((data: any) => {
    if (data && data.status === 200) {
      this.spinner.hide();
      this.success(this.util.getString('Delivery range changed successfully'));
    }else{
      this.spinner.hide();
       this.error(this.util.getString('Something went wrong'));
    }
...
//   this.api.post('stores/editList', param).then((data: any) => {
//     if (data && data.status === 200) {
//             console.log('getDeliveryRange:getDeliveryRange==',data)
//     }
//   })
// }

/**  closed Delivery Range add here in store table  */
}
